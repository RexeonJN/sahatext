...
exports.postSignup = (req, res, next) => {
  const radio = req.body.gridRadios;
  const name = req.body.name; 
  const email = req.body.email; 
  const pass = req.body.pwd;
  const cpass = req.body.cpwd; 
  ...
  if (!errors.isEmpty()) {
   ...
   return Brand.findOne({ where: { brand_name: 'turtle' }})
   .then(b =>{
     b.getProducts({limit: 4})
     .then(bpro => {
       brandlist = bpro;
    ...
    User.findOne({ where : { email: req.body.email } })
      ...
      .then(result => {
        ...
        transporter.sendMail({
          to: req.body.email,
          from: 'kundankumar5037@gmail.com',
          subject: 'Password reset',
          html: `
            <p>You requested a password reset</p>
            <p>Click this <a href="https://powerful-brook-05214.herokuapp.com/reset/${token}">link</a> to set a new password.</p>
          `
        });
      })
      .catch(err => {
